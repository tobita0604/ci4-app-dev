								...
								<?php foreach ($members as $member) : ?>
									<tr <?= !empty($member['R01_Cancel_Flg']) ? 'hidden' : '' ?>>
										...
										<td>
											<?php
												...
												if(!empty($member['R02_Option1_Time'])) {
													$time = v($time_val, $member['R02_Option1_Time']);
												}
											 ?>
										...
										<td>
											<?php
												...
												if(!empty($member['R02_Option2_Time'])) {
													$time = v($time_val, $member['R02_Option2_Time']);
												}
											 ?>
										...
										<!-- <td>
											<?php
												$time = "";
												if(!empty($member['R02_Option3_Time'])) {
													$time = v($time_val, $member['R02_Option3_Time']);
												}
											 ?>
											<?= !empty($member['R02_Option3']) ? v($schedule, $member['R02_Option3']) : '不参加' ?>
											<?php if (!empty($time)) : ?>
												<span class="time" <?= empty($member['R02_Option3_Time']) ? 'hidden' : '' ?>>
													<br>時間
													<?= $time ?>
												</span>
											<?php endif; ?>
...
?>
						&nbsp;&nbsp;&nbsp;
						<a href="<?php echo base_url(); ?>option_con/option_7" style="margin-bottom: 10px;" class="btn btn--orange">オプショナルツアー申込み</a>
<?php
	}
?>
						</div>
			...
			<div id="contents">
				<section class="detail op_sp">
					<p><strong>【注意事項について】</strong><br />
					●同伴者登録後、参加者のお名前・年齢が表示されます。<br />
					●「オプショナルツアー申込み」ボタンからお申込みができます。<br />
					●申込みいただいた内容をキャンセルする場合は、再度申込み画面へ進み「申込み」を「不参加」へ戻し登録ボタンを押してください。<br>
<!--
					<h2>オプショナルツアー</h2>
