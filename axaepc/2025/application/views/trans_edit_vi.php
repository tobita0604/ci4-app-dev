...
<div class="contents">
	...
	<section id="about">
		<div id="main">
			...
			<form action="<?php echo base_url(); ?>Transfer_con/save" method="post" autocomplete="off" id="option_data">
				<h2 style="background:#005084">EPC2024 移動方法の登録　申込</h2>
				...
				<table class="border2" width="100%" style="table-layout:fixed">
					...
					<?php foreach ($members as $member) : ?>
						<tr <?= !empty($member['R01_Cancel_Flg']) ? 'hidden' : '' ?>>
							...
							<td>
								<select name="R02_bus_dep[<?= $member['R01_seq'] ?>]" class="form-control">
									<option value="0" <?= $member['R02_bus_dep'] == 0 ? 'selected' : '' ?>>(未登録)</option>
									<option value="1" <?= $member['R02_bus_dep'] == 1 ? 'selected' : '' ?>>シャトルバス</option>
									<option value="2" <?= $member['R02_bus_dep'] == 2 ? 'selected' : '' ?>>レンタカー（各自予約）</option>
								</select>
							</td>
							<td>
								<select name="R02_bus_arr[<?= $member['R01_seq'] ?>]" class="form-control">
									<option value="0" <?= $member['R02_bus_arr'] == 0 ? 'selected' : '' ?>>(未登録)</option>
									<option value="1" <?= $member['R02_bus_arr'] == 1 ? 'selected' : '' ?>>シャトルバス</option>
									<option value="2" <?= $member['R02_bus_arr'] == 2 ? 'selected' : '' ?>>レンタカー（各自予約）</option>
								</select>
							</td>
						</tr>
					<?php endforeach; ?>
