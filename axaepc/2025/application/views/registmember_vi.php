...
<form action="<?php echo base_url();?>register_con/save_member" method="post" autocomplete="off" id="entry_data" > 
	...
	$no = $idx + 1;
	?>
	<h2 style ="background:#005084">EPC2024　エントリー　同伴者<?=$no?>　登録</h2>
...
-->
	...
	<table class="border2 invite <?=!empty($invites[$idx]['R01_Cancel_Flg'])?'cxl':''?> <?=$gai?>" id="invite<?=$no?>">
		...
		<tr>
			...
			<td>
				<input readonly type="text" name="invite[R01_Age]" value="<?=h($invites[$idx]['R01_Age'])?>" size="2" class="digits"/>　<span style="color:red;">※<?=date('Y年m月d日',strtotime(DEPARTURE_DATE));?>基準</span>
		...
		<tr>
			<th colspan="2">
				<input type="checkbox" name="invite[copy]" value="1" onchange="copy_checked(this)" />代表者と同じ　<span style="color:red;">下記内容がご本人と同じ場合、チェックをいれてください。</span>
			</th>
		</tr>
		<tr class="copy">
		...
		<tr class="copy">
			...
			<td>
				<input type="text" name="invite[R01_Emer_Name]" value="<?=h($invites[$idx]['R01_Emer_Name'])?>" />　<span style="color:red;">※同行されないご家族の方に限ります。</span>
...
<!--
		...
		<tr class="infant-bassinet">
			<th>＜往路　新幹線利用＞<br><span style="color:red;">【東北・中部・関西発の方のみ必須】</span><br><a href="<?php echo base_url();?>pdf/Transportation_ EPC2024.pdf" target="_blank"><span style="color:#000; font-weght: bold;"><u>支社別 交通移動一覧</u></span></a></th>
			<td>
...
&nbsp;&nbsp;&nbsp;
	<input type="image"  src="<?php echo base_url();?>img/next.png" onclick="return validate_member();"/>
<?php else:?>
	<input type="image"  src="<?php echo base_url();?>img/back.png" onclick="go_back_entry_no();"/>
&nbsp;&nbsp;&nbsp;
	<input type="image"  src="<?php echo base_url();?>img/save.png" onclick="return validate_member(1);"/>
<?php endif;?>
