...
<?php } }?>
<form action="<?php echo base_url(); ?>menu_con/search" method="post" name="search" id="search">
<?php require(APPPATH . "views/element/csrf_input.php"); ?>
...
<?php if($this->session->userdata('charger_type')=='9'): ?>
<div style="margin: 0 auto; margin-left: 5px">
	<span style="float:left;">※マイページを参照するには社員番号をクリックしてください。</span>
</div>
<?php endif;?>
<!-- CSV END -->
...
<table class="search" width="100%">
	...
	?>
	<tr bgcolor="<?php echo $bgc; ?>" style="background-color:<?php echo $bgc; ?>;">
		<td style="text-align:center;white-space:nowrap" <?php if($this->session->userdata('charger_type')=='9'): ?>onclick="openOtherEditPage('<?=$data['R01_Id']?>','<?= $this->security->get_csrf_token_name(); ?>','<?= $this->security->get_csrf_hash(); ?>')"<?php endif;?>>
			<?=$data['R01_Id'] ?>
		</td>
		<td>
		...
		<td <?php if($this->session->userdata('charger_type')=='9'): ?>onclick="openNoteEditPage('<?=$data['R01_Id']?>','<?= $this->security->get_csrf_token_name(); ?>','<?= $this->security->get_csrf_hash(); ?>')"<?php endif;?>>
			編集&nbsp;
			<?php if(!empty($data['R01_note'])){echo '(ﾒﾓ有)';} ?>
		</td>
		<td <?php if($this->session->userdata('charger_type')=='9'): ?>onclick="openChargepaxPage('<?=$data['R01_Id']?>','<?= $this->security->get_csrf_token_name(); ?>','<?= $this->security->get_csrf_hash(); ?>')"<?php endif;?>>
			変更&nbsp;
		</td>
		<?php endif;?>
	</tr>
