...
class Mypage_admin_con extends CI_Controller {
	...
	public function index() {
		...
		if(check_login_user_session()){
			...
			$this->load->view('registno_admin_vi',$data);
	...
	public function admin_view() {
		...
		redirect(base_url("mypage_admin_con"));
	}
	...
	public function note_view() {
		...
		if(check_login_user_session()){
			...
			$reserve = $this->reserve_mo->get_reserver_info($R01_Id);
			$invites = $this->reserve_mo->get_all_members_info($R01_Id);
			$note = $this->reserve_mo->get_note($R01_Id);
			...
			$this->load->view('head_vi');
			$this->load->view('session_timeout_vi');
			$this->load->view('note_admin_vi',$data);
			$this->load->view('last_vi');
			// $this->load->view('aside_mypage_vi');
			$this->session->set_userdata('sysmsg', '');
		}else{
			redirect(base_url("login_con"));
		}		
	...
	public function chargepax_view() {
		$R01_Id = $this->input->post('R01_Id');
		if(empty($R01_Id)){
			$R01_Id = $this->session->userdata('user_data');
		}
		...
		if(check_login_user_session()){
			...
			$this->load->view('head_vi');
			$this->load->view('session_timeout_vi');
			$this->load->view('chargepax_admin_vi',$data);
			$this->load->view('last_vi');
			// $this->load->view('aside_mypage_vi');
			$this->session->set_userdata('sysmsg', '');
		}else{
	...
	public function save_entry_no() {
		if(check_login_user_session()){
			...
			redirect(base_url("mypage_admin_con/regist_reserver"));
		}else{
			redirect(base_url("login_con"));
		}
	}
	...
	public function regist_reserver() {
		...
		if(check_login_user_session()){
			...
			$this->load->view('header_vi',$data);
			$this->load->view('session_timeout_vi');
			$this->load->view('registreserver_admin_vi',$data);
	...
	public function regist_member() {
		...
		if(check_login_user_session()){
			...
			$this->load->view('header_vi',$data);
			$this->load->view('session_timeout_vi');
			$this->load->view('registmember_admin_vi',$data);
			$this->load->view('regist_script_vi',$data);
			// $this->load->view('aside_mypage_vi');
			$this->load->view('footer_vi');
	...
	// 		if($invites_no == 1) {
	// 			redirect(base_url("mypage_admin_con/confirm"));
	// 		} else {
	// 			redirect(base_url("mypage_admin_con/confirm"));
	// 		}
	...
	// 			redirect(base_url("Mypage_admin_con/regist_member"));
	// 		} else {
	// 			redirect(base_url("Mypage_admin_con/confirm"));
	// 		}
	// 	}else{
	...
	private function backup() {
		...
		// 管理画面から更新した場合、更新ログを残す
		$admin = $this->session->userdata('admin_id_session');
		$today = date("Y-m-d H:i:s");
		$filename = 'admin_update_log.txt';
		$content = $today.' => update_user：'.$admin.PHP_EOL;
		$fp = fopen($filename,'a');
		fwrite($fp,$content);
		fclose($fp);
	}
