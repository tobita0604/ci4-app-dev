...
class Option_con extends CI_Controller
{
	...
	public function index()
	{
		...
		if (check_login_user_session()) {
			...
			$time_vals = $this->option_mo->get_times(0, 0);
			$schedule3 = $this->option_mo->get_schedule(0);

			// 表示用配列整形
			foreach ($time_vals as $v) {
			...
			$this->load->view('option_view_vi', $data);
	...
	public function option_7()
	{
		...
		if (check_login_user_session()) {
			...
			$schedule3 = $this->option_mo->get_schedule(3);
			...
			foreach ($schedule1 as $indx1 => $s1) {
				$schedule1[$indx1]['M01_Day1_Time'] = [];
				$option_time_23 = $this->option_mo->get_times($s1['M01_Id'], 1);
				foreach ((array)$option_time_23 as $time_23) {
					// $schedule1[$indx1]['M01_Day1_Time'][$time_23['M01_Stock_Id']] = $time_23['M01_Time_Text'];
					$schedule1[$indx1]['M01_Day1_Time'][0] = '';
				}
			}

			foreach ($schedule2 as $indx2 => $s2) {
				$schedule2[$indx2]['M01_Day2_Time'] = [];
				$option_time_24 = $this->option_mo->get_times($s2['M01_Id'], 2);
				foreach ((array)$option_time_24 as $time_24) {
					// $schedule2[$indx2]['M01_Day2_Time'][$time_24['M01_Stock_Id']] = $time_24['M01_Time_Text'];
					$schedule2[$indx2]['M01_Day2_Time'][0] = '';
				}
			}

			foreach ($schedule3 as $indx3 => $s3) {
				$schedule3[$indx3]['M01_Day3_Time'] = [];
				$option_time_25 = $this->option_mo->get_times($s3['M01_Id'], 3);
				foreach ((array)$option_time_25 as $time_25) {
					// $schedule3[$indx3]['M01_Day3_Time'][$time_25['M01_Stock_Id']] = $time_25['M01_Time_Text'];
					$schedule3[$indx3]['M01_Day3_Time'][0] = '';
				}
			}
	...
	public function option_4q()
	{
		if (check_login_user_session()) {
			...
			foreach ($schedule1 as $indx1 => $s1) {
				$schedule1[$indx1]['M01_Day1_Time'] = '';
				$option_time_23 = $this->option_mo->get_times($s1['M01_Id'], 1);
				foreach ($option_time_23 as $time_23) {
					$schedule1[$indx1]['M01_Day1_Time'][$time_23['M01_Stock_Id']] = $time_23['M01_Time_Text'];
				}
			}
			...
			foreach ($schedule2 as $indx2 => $s2) {
				$schedule2[$indx2]['M01_Day2_Time'] = '';
				$option_time_24 = $this->option_mo->get_times($s2['M01_Id'], 2);
				foreach ($option_time_24 as $time_24) {
					$schedule2[$indx2]['M01_Day2_Time'][$time_24['M01_Stock_Id']] = $time_24['M01_Time_Text'];
				}
			}
	...
	public function save_option()
	{
		if (check_login_user_session()) {
			$update_flg = false;
			$R01_Id = $this->session->userdata('user_data');
			$options_conf = $this->option_mo->get_options_info($R01_Id);
			$members = $this->option_mo->get_members_info($R01_Id);
			$options = $this->option_mo->get_options();
			$view_flg = $this->input->post('view_flg');
			// foreach ($options as $option) {
			// 	$prices[$option['M01_Id']] =  $option;
			// }
			$options = $this->input->post('option');
			...
			// 	}
			// 	if (empty($count1)) {
			// 		$time_srt1 = $this->stockNoTimeReserve($o['R02_Option1'], 1);
			// 	}
			...
			// 	}
			// 	if (empty($count2)) {
			// 		$time_srt2 = $this->stockNoTimeReserve($o['R02_Option2'], 2);
			// 	}
			...
			// 更新処理前の在庫check
			// $stock = $this->stock_lib->balanceBeforeUpdateCheck($options);
			// if ($stock == true) {
			// 	$this->session->set_tempdata('success_flash', '既に予約が満員に達したツアーがあるため、処理を中断しました。', 1);
			// 	if ($view_flg == 4) {
			// 		redirect(base_url("option_con/option_4q"));
			// 	} else if ($view_flg == 7) {
			// 		redirect(base_url("option_con/option_vi"));
			// 	}
			// }
			...
			// 更新処理
			if ($update_flg) {
				$this->stockReturn($members);
				foreach ($options as $idx => $option) {
					$option['R02_Price'] = $this->option_mo->getOptionPrice($option, $R01_Id, $idx);
					$this->option_mo->update($option, ['R02_Id' => $R01_Id, 'R02_seq' => $idx]);
					// $this->stockReserve($option['R02_Option1_Time']);
					// $this->stockReserve($option['R02_Option2_Time']);
				}
			} else {
				foreach ($options as $idx => $option) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->session->set_tempdata('success_flash', 'システムエラー：DB更新処理に失敗しました。<br>入力データの更新を中断します。', 1);
	...
	public function save_jihi()
	{
		if (check_login_user_session()) {
			...
			$members = $this->option_mo->get_members_info($R01_Id);
			$jihi_flg = $this->input->post('jihi_flg');
			...
			$this->session->set_tempdata('success_flash', '更新完了しました。', 1);
			if (!empty($jihi_flg)) {
				redirect(base_url("option_con/jihi_complete"));
			} else {
				redirect(base_url("option_con/complete"));
			}
		} else {
	...
	// 自費参加
	public function jihi_complete()
	{
		if (check_login_user_session()) {
			$this->load->view('head_vi');
			$this->load->view('header_vi');
			$this->load->view('option_jihi_complete_vi');
			$this->load->view('aside_mypage_vi');
			$this->load->view('footer_vi');
		} else {
			redirect(base_url("login_con"));
	...
	//==============================================
	// 時間の指定が無い在庫を取得（引き落とし準備）
	//==============================================
	private function stockNoTimeReserve($id, $type)
	{
		if (!empty($id)) {
			$stocks = $this->option_mo->getNoTimeStock($id, $type);
			if (!empty($stocks)) {
				foreach ($stocks as $stock) {
					if ($stock['M01_Id'] == $id) {
						return $stock['M01_Stock_Id'];
					}
				}
			}
		}
	...
	// オプショナルツアー在庫引き落とし
	//==============================================
	private function stockReserve($id)
	{
		if (!empty($id)) {
			$stock = $this->option_mo->getOptionsStock($id);
			$stock['M01_Reserve'] += 1;
			$stock['M01_Balance'] -= 1;
			unset($stock['M01_Stock_Id']);
			$this->option_mo->updateStock($stock, ['M01_Stock_Id' => $id]);
		}
	}
	...
	// オプショナルツアー在庫戻し
	//==============================================
	private function stockReturn($members)
	{
		if (!empty($members)) {
			foreach ($members as $member) {
				if (!empty($member['R02_Option1_Time'])) {
					$stock1 = $this->option_mo->getOptionsStock($member['R02_Option1_Time']);
					$stock1['M01_Reserve'] -= 1;
					$stock1['M01_Balance'] += 1;
					unset($stock1['M01_Stock_Id']);
					$this->option_mo->updateStock($stock1, ['M01_Stock_Id' => $member['R02_Option1_Time']]);
				}

				if (!empty($member['R02_Option2_Time'])) {
					$stock2 = $this->option_mo->getOptionsStock($member['R02_Option2_Time']);
					$stock2['M01_Reserve'] -= 1;
					$stock2['M01_Balance'] += 1;
					unset($stock2['M01_Stock_Id']);
					$this->option_mo->updateStock($stock2, ['M01_Stock_Id' => $member['R02_Option2_Time']]);
				}
			}
		}
	}
